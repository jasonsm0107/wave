...
// Function to handle trading pair changes
function initPairSelector() {
    const pairSelector = document.querySelector('.pair-dropdown-btn');
    
    if (pairSelector) {
        // Create the dropdown content
        const dropdownContent = document.createElement('div');
        dropdownContent.className = 'pair-dropdown-content';
        dropdownContent.style.display = 'none';
        
        // Define popular trading pairs
        const popularPairs = [
            { symbol: 'BTCUSDT', name: 'Bitcoin', icon: 'assets/btc-icon.svg' },
            { symbol: 'ETHUSDT', name: 'Ethereum', icon: 'assets/eth-icon.svg' },
            { symbol: 'SOLUSDT', name: 'Solana', icon: 'assets/sol-icon.svg' },
            { symbol: 'BNBUSDT', name: 'Binance Coin', icon: 'assets/bnb-icon.svg' },
            { symbol: 'ADAUSDT', name: 'Cardano', icon: 'assets/ada-icon.svg' },
            { symbol: 'DOGEUSDT', name: 'Dogecoin', icon: 'assets/doge-icon.svg' }
        ];
        ...
        popularPairs.forEach(pair => {
            ...
            item.innerHTML = `
                <div class="pair-icon small">
                    <img src="${pair.icon}" alt="${pair.name}" onerror="this.src='assets/default-coin.svg'">
                </div>
        ...
        // Insert dropdown content into the DOM
        pairSelector.parentNode.style.position = 'relative';
        pairSelector.parentNode.appendChild(dropdownContent);
...
// Function to change trading symbol
function changeSymbol(pair) {
    // Update pair info in the UI
    const pairTitle = document.querySelector('.pair-details h2');
    const pairIcon = document.querySelector('.pair-icon img');
    
    if (pairTitle) {
        pairTitle.textContent = pair.symbol;
    }
    
    if (pairIcon) {
        pairIcon.src = pair.icon;
        pairIcon.alt = pair.name;
    }
    
    // Show loading indicators
    const orderbookContent = document.querySelector('.orderbook-content');
    if (orderbookContent) {
        orderbookContent.classList.add('loading');
    }
    
    // Update the TradingView chart symbol
    if (window.tradingViewWidget) {
        window.tradingViewWidget.setSymbol(`BINANCE:${pair.symbol}`, {
            interval: document.querySelector('.btn-time.active')?.textContent.trim() || '15m'
        });
    }
    
    // Update the trading data service
    tradingDataService.changeSymbol(pair.symbol);
    
    // Show notification
    const notification = document.createElement('div');
    notification.className = 'symbol-change-notification';
    notification.innerHTML = `
        <div class="notification-icon">
            <img src="${pair.icon}" alt="${pair.name}" onerror="this.src='assets/default-coin.svg'">
        </div>
        <div class="notification-content">
            <div class="notification-title">Symbol Changed</div>
            <div class="notification-message">Now trading ${pair.name} (${pair.symbol})</div>
        </div>
    `;
    ...
    setTimeout(() => {
        if (orderbookContent) {
            orderbookContent.classList.remove('loading');
        }
    }, 1500);
}

// Initialize pair selector after DOM loaded
document.addEventListener('DOMContentLoaded', () => {
    initPairSelector();
});